...
- create_or_retrieve_knowledge_base: Creates a new Knowledge Base or retrieves an existing one.
...
from opensearchpy import (AWSV4SignerAuth, OpenSearch, RequestError,
                          RequestsHttpConnection)
from retrying import retry

sys.path.append('../')
from util.model_constants import DEFAULT_EMBEDDING_MODEL, EMBEDDING_MODELS
from util.tagging import standard_tags, standard_tags_kv

valid_embedding_models = list(EMBEDDING_MODELS.keys())
...
class KnowledgeBasesForAmazonBedrock:
    ...
    def create_or_retrieve_knowledge_base(
        ...
        """
        Function used to create a new Knowledge Base or retrieve an existing one
    ...
    def create_oss(self, vector_store_name: str, oss_policy_name: str, bedrock_kb_execution_role: str):
        """
        Create OpenSearch Serverless Collection. If already existing, retrieve
        Args:
            vector_store_name: name of the vector store
    ...
    def create_vector_index(self, index_name: str):
        """
        Create OpenSearch Serverless vector index. If existing, ignore
        Args:
    ...
    def create_knowledge_base(
        ...
        """
        Create Knowledge Base and its Data Source. If existing, retrieve
        Args:
